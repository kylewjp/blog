...
class orderControl {
    ...
    public function _create_order_statis() {
		...
		//原版
        // $start_unixtime = strtotime(date('Y-m-01 00:00:00', $start_unixtime));//print_r(date('Ymd', $start_unixtime));exit;
        // while (($time = strtotime('-'.$i.' month')) >= $start_unixtime) {//print_r(date('Ymd', $start_unixtime));exit;
...
//七天----------------------------			
		$w = date('w', $start_unixtime);
		if($w == 0){
			$Monday = strtotime(date('Y-m-d 00:00:00', $start_unixtime - 86400 * 6));
			$Sunday = strtotime(date('Y-m-d 23:59:59', $start_unixtime));
		}else{
			$Monday = strtotime(date('Y-m-d 00:00:00', $start_unixtime - 86400 * ($w - 1)));//print_r(date('Y-m-d H:i:s',$Monday));exit;
			$Sunday = strtotime(date('Y-m-d 23:59:59', $start_unixtime - 86400 * ($w - 7)));//print_r(date('Y-m-d',strtotime('-'.($i * 7).' day')));exit;print_r(date('Y-m-d',$Sunday));exit;
		}
		
		while (($time = strtotime('-'.($i * 7).' day')) > $Monday) {//print_r(date('Ymd H:i:s',$time));exit;
			$now = date('w', $time);
			if($now == 0){
				$nowMonday = $time - 86400 * 6;
				$nowSunday = $time;
			}else{
				$nowMonday = $time - 86400 * ($now - 1);
				$nowSunday = $time - 86400 * ($now - 7);
			}
			
			$first_day_unixtime = strtotime(date('Y-m-d 00:00:00', $nowMonday));
            $last_day_unixtime = strtotime(date('Y-m-d 23:59:59', $nowSunday));
//七天----------------------------	
            ...
            $data[$key]['os_month'] = date('Ymd', $first_day_unixtime);
			
            $data[$key]['os_year'] = date('Y',$first_day_unixtime);
            $data[$key]['os_start_date'] = $first_day_unixtime;
            $data[$key]['os_end_date'] = $last_day_unixtime;//print_r($data);exit;
            ...
            $order_bill_info = $model_bill->getOrderBillInfo(array('ob_start_date'=>$data[$key]['os_start_date']),$fileds);
