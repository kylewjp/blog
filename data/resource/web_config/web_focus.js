...
function add_screen() {//增加图片
	...
	for (var i = 1; i <= screen_max; i++) {//防止数组下标重复
		if (screen_obj.find("li[screen_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			var add_html = '';
			add_html = '<li screen_id="'+i+'" onclick="select_screen('+i+');" title="111可上下拖拽更改显示顺序"><a class="del" href="JavaScript:del_screen('+i+
	...
	for (var i = 1; i <= focus_max; i++) {//防止数组下标重复
		...
		if (focus_obj.find("div[focus_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			for (var pic_id = 1; pic_id <= pic_max; pic_id++) {
			    text_append += '<li pic_id="'+pic_id+'" onclick="select_focus('+i+',this);" title="可左右拖拽111更改图片排列顺序">';
			...
			add_html = '<div focus_id="'+i+'" class="focus-trigeminy-group" title="可上下拖拽更改显示2顺序"><a class="del" href="JavaScript:del_focus('+i+');" title="删除">X</a>'+
