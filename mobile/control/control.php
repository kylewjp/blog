...
class Control{
	...
	//通过ip获取地址信息
	public function getIP2address($ip){
		$info1=$this->geturl('http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=json&ip='.$ip);
		//解析json
		...
		$info['district'] = $info1['district'];
		$info['isp'] = $info1['isp'];
		$info['type'] = $info1['type'];
		$info['desc'] = $info1['desc'];
		return $info;
	}
	//直接获取当前地址信息
	public function getNowAddress(){
		$onlineip =$this->getIp();
		if($onlineip ='127.0.0.1'){
			$onlineip ='119.29.55.207';
		}
		$info=$this->getIP2address($onlineip);
		$info['ip'] = $onlineip;
		return $info;
	}
	//获取当前ip
...
class BaseHomeControl extends Control {
	...
	public function __construct(){
		...
		if ($_GET['column'] && strtoupper(CHARSET) == 'UTF8'){
    ...
    private function _getMenuList() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
                array('name' => '我要进货', 'act'=>'sys_goods_add', 'op'=>'index'),
            ...
            'promotion' => array('name' => '促销', 'child' => array(
                ...
                array('name' => '专题活动', 'act'=>'wjp_store_project_activity', 'op'=>'project_activity_list'),
                array('name' => '图片资讯', 'act'=>'wjp_sent_article', 'op'=>'index'),
            )),
            'store' => array('name' => '店铺', 'child' => array(
                ...
                array('name' => '微信设置', 'act'=>'store_wechat', 'op'=>'setting'),
                array('name' => '店铺分类', 'act'=>'store_goods_class', 'op'=>'index'),
                array('name' => '品牌申请', 'act'=>'store_brand', 'op'=>'brand_list'),
                array('name' => '微店审批', 'act'=>'wjp_wechat_shop', 'op'=>'shop_list'),
            )),
		...
		$store_grade_info = Model('store_grade')->getOneGrade($_SESSION['grade_id']);
		if(!$store_grade_info['sg_add_store_goods'])
		{
			unset($menu_list['goods']['child'][1]);
		}
		if($_SESSION['store_id'] == DEFAULT_PLATFORM_STORE_ID)
		{
			unset($menu_list['goods']['child'][0]);
		}
            ...
            if(!empty($picture_detail)) {
                if( $this->publisher_type == self::ARTICLE_TYPE_ADMIN ||
                    ($picture_detail['picture_type'] != self::ARTICLE_TYPE_ADMIN
                     && $picture_detail['picture_publisher_id'] == $_SESSION['member_id'])) {
 ...
 * 会员API新类
 */
class BaseMenberapiControl extends Control {
	public function __construct(){
