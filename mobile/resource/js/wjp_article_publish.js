 ...
 //图片上传
    $("#article_image_upload").fileupload({
        dataType: 'json',
            url: "index.php?act=wjp_sent_article&op=article_image_upload",
            add: function(e,data) {
    ...
    $("[nctype='btn_drop_article_image']").live("click",function(){
        ...
        $.getJSON("index.php?act=wjp_sent_article&op=article_image_drop", { image_name: image_name }, function(result){
            if(result.status == "success") {
                image_object.remove();
            } else {
                showError(result.error);
