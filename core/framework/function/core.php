...
function showMessage($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=2000){
	...
	switch ($show_type){
		...
		case 'exception':
			...
			echo '<dd><p /><p /><p /><p /></dd>';
			echo '<dd><p /><p /><p /><p />Copyright 2007-2014 mphelper, All Rights Reserved '.$lang['company_name'].'</dd>';
			echo '</dl>';
			echo '</body>';
			echo '</html>';
			exit;
...
function getNchash(){
    if (C('captcha_status_login')){
        return substr(md5(MAIN_SITE_URL.$_GET['act'].$_GET['op']),0,8);
    } else {
        return '';
    }
}
	...
	if (is_dir($path)){
		...
		if (!empty($file_list)){
			foreach ($file_list as $v){
				if (basename($v) != 'index.php')@unlink($v);
			}
		}
	}else{
		if (basename($path) != 'index.php') @unlink($path);
	}
	return true;
}
 ...
 * 获取手机版画报URL
 */
function getCMSPhonePictureUrl($picture_id) {
    if(URL_MODEL) {
        // 开启伪静态
        return CMS_SITE_URL.DS.'picture-'.$picture_id.'.html';
    } else {
        return CMS_SITE_URL.DS.'index.php?act=picture&op=picture_detail_phone&picture_id='.$picture_id;
    }
}
...
function snsShareFrom($sign) {
    switch ($sign) {
        case '1' :
        case '2' :
            return L('sns_from') . '<a target="_black" href="' . MAIN_SITE_URL . '">' . L('sns_shop') . '</a>';
            break;
        case '3' :
            return L('sns_from') . '<a target="_black" href="' . MICROMAIN_SITE_URL . '">' . L('nc_modules_microshop') . '</a>';
            break;
        case '4' :
...
function url($act = '', $op = '', $args = array(), $model = false, $site_url = ''){
    ...
    //入口文件名
    $file = 'index.php';
//    $site_url = empty($site_url) ? MAIN_SITE_URL : $site_url;
    $act = trim($act);
    $op = trim($op);
...
function urlShop($act = '', $op = '', $args = array(), $store_domain = ''){
    ...
    return url($act, $op, $args, $rewrite_flag, MAIN_SITE_URL);
}
