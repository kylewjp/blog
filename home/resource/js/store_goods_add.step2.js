...
// 计算返利率
function rebate_percentCalculator() {
    var _price = parseFloat($('input[name="g_price"]').val());
    var _rebate = parseFloat($('input[name="g_rebate"]').val());
    if((!isNaN(_price) && _price != 0) && (!isNaN(_rebate) && _rebate != 0)){
        var _rebate_percent = parseInt(_rebate/_price*100);
        $('input[name="g_rebate_percent"]').val(_rebate_percent);
    }
}
 ...
 * 2014/11/20 15:45
 */
function super_bonus_percentCalculator() {
    var _price = parseFloat($('input[name="g_price"]').val());
    var _bonus = parseFloat($('input[name="super_bonus"]').val());
    if((!isNaN(_price) && _price != 0) && (!isNaN(_bonus) && _bonus != 0)){
        var _bonus_percent = parseInt(_bonus/_price*100);
        $('input[name="super_bonus_percent"]').val(_bonus_percent);
    }
}
//获得商品名称
